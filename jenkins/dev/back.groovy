pipeline {
  agent any
    ...
    environment {
            ...
            repo = "https://github.com/ProjectTeam-Test/Back.git"
            buildName = "dev_back"
            build = "back"
            branch = 'main'
            ...
            steps {
                script {
                    def blue_container_id = sh(
                            script: "docker ps --filter name=dev-back-blue -q",
                            returnStdout: true
                    ).trim()
                    ...
                    if (blue_container_id) {
                        ...
                        sh "docker exec dev-back-nginx sed -i 's|http://3.35.222.131:8091|http://3.35.222.131:8092|g' /etc/nginx/nginx.conf"
                        sh "docker-compose -f docker-compose.blue.yml down"
                        ...
                        sh "docker exec dev-back-nginx sed -i 's|http://3.35.222.131:8092|http://3.35.222.131:8091|g' /etc/nginx/nginx.conf"
                        sh "docker-compose -f docker-compose.green.yml down"
                    }
                    
                    sh "docker restart dev-back-nginx"
                }
            }
        }
    }
