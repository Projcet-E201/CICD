pipeline {
  agent any
    ...
    environment {
            ...
            repo = "https://github.com/ProjectTeam-Test/Data.git"
            buildName = "dev_data"
            build = "data"
            branch = 'main'
            ...
            steps {
                script {
                    ...
                    if (blue_container_id) {
                        ...
                        sh "docker exec dev-data-nginx sed -i 's|http://3.35.222.131:8097|http://3.35.222.131:8098|g' /etc/nginx/nginx.conf"
                        sh "docker-compose -f docker-compose.blue.yml down"
                        ...
                        sh "docker exec dev-data-nginx sed -i 's|http://3.35.222.131:8098|http://3.35.222.131:8097|g' /etc/nginx/nginx.conf"
                        sh "docker-compose -f docker-compose.green.yml down"
                    }
                    
                    sh "docker restart dev-data-nginx"
                }
            }
        }
    }
